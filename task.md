...
💻 アーキテクチャ/コード品質 P0
[x] 状態管理の一貫: 全画面をRiverpodのref.watchへ接続（モック除去）
...
[x] EXIF/PII対策: 写真保存時にEXIF削除・サムネ圧縮・ファイル名ハッシュ化

[x] バックアップ方針: ユーザーデータのエクスポート/削除APIとUI（必須：Apple審査）

[x] 時刻一貫性: 端末時刻改変耐性（NTPライク簡易検査 or サーバ基準）
...
[x] アカウント削除（アプリ内）: ワンタップで申請→即時/一定期間後の削除実行（Apple 5.1.1対応）

🔔 通知/バックグラウンド P0
[x] タイムゾーン対応: ユーザー移動時の再スケジュール

[x] 重複通知防止: 同一クエストの近接通知をまとめる

[x] 押下ディープリンク: 通知→Record対象へ直接遷移

[x] 失敗ハンドリング: 権限拒否時の代替（ホーム上バナー/翌朝一括）
...
[x] KPIダッシュボード: D1/D7留存、日次完了率、通知開封率、クラッシュ率

[x] Remote Config: 通知時刻の初期値/本数、推奨クエスト配信のAB実験
...
[x] 短文/長文説明: 一貫メッセージ、主要キーワード（習慣化/匿名/3タップ/ペア）

[ ] 年齢レーティング: 各質問票の準備（暴力・ユーザー生成コンテンツの扱い）

[x] コンテンツ権利: 素材/フォント/効果音のライセンス表記ページ（OSS含む）

[x] アプリ内サポート: 問い合わせ（メール/フォーム）とFAQリンク
...
[x] 障害対応Runbook: 重大バグ時のフラグOFF→ホットフィックス→連絡テンプレ

[x] 監視: クラッシュ/ANR/パフォーマンス/通知配信の常時監視設定
...
✨ P1（推奨・MVP後に強化）
[x] SIWA（Sign in with Apple）対応の要否整理（iOSでGoogleを出す場合）

[x] 端末バックアップ/機種変更引継ぎUX（匿名→後付け連携）

[x] リッチ統計: 週次トレンド、カテゴリ別達成率、自己比較

[x] フィードバック収集: アプリ内NPS/問い合わせフォーム

[x] アプリアイコンAB（Remote Configで切替）

[x] 多言語展開（英語/韓国語など）とストア翻訳

[x] オンデバイス軽微モデレーション（不適切画像の疑義検出）

[x] マーケ計測（UTMディープリンク、アトリビューションSDK検討）

[x] 収益化実験（広告は体験阻害大：まずは寄付/サブスクの準備だけ）
...
[x] ストア審査質問票（データ安全/年齢/コンテンツ）に一切の矛盾なし
